...
describe("server wall-clock (no open tab)", () => {
  it("advances from last_move_at even without a client tick", () => {
    ...
    expect(clocksAreRunning(["e2e4", "e7e5"], last)).toBe(true);
    const live = interpolateClocks(180_000, 180_000, last, "w", {
      moves: ["e2e4", "e7e5"],
    });
    expect(live.whiteTimeLeft).toBe(135_000);
    expect(live.blackTimeLeft).toBe(180_000);
  });
